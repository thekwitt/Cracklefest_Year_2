...
module.exports = {
	name: 'addcoins',
	description: 'Add coins to someone\'s inventory!',
	data: new SlashCommandBuilder()
		.setName('addcoins')
		.setDescription('Add coins to someone\'s inventory!')
		.addSubcommand(subcommand =>
			subcommand
				.setName('user')
				.setDescription('Add coins to a user.')
				.addUserOption(option => option.setName('target').setDescription('The Person you want to add coins to.').setRequired(true))
				.addIntegerOption(option => option.setName('amount').setDescription('How many coins?').setRequired(true)))
		.addSubcommand(subcommand =>
			subcommand
				.setName('role')
				.setDescription('Add coins to users of a role.')
				.addRoleOption(option => option.setName('role').setDescription('The role you want to add coins to.').setRequired(true))
				.addIntegerOption(option => option.setName('amount').setDescription('How many coins?').setRequired(true))),
	...
	async execute(interaction, client) {
		...
		if(amount > 100 || amount < 1)
		{
			...
			try{return await interaction.reply({ content: 'You can only add 1 - 100 coins at a time!', ephemeral: true }).then(client.extra.log_g(client.logger, interaction.guild, this.name + ' Command', 'No gift Warning Reply'));}
		...
		if(subcommand == 'user') {
			...
			await client.pool.query('UPDATE user_data SET gold_coins = gold_coins + $1 WHERE Guild_ID = $3 AND Member_ID = $2;', [amount, target.id, guildId]);

			const embed = new MessageEmbed()
				.setColor(client.colors[0][2])
				.setTitle(target.username + ' got ' + amount + ' coin' + (amount > 1 ? 's' : '') + '!')
				// eslint-disable-next-line spaced-comment
				//.setThumbnail(user.defaultAvatarURL)
				.setDescription('⠀\n' + target.username + ' got ' + amount + ' coin' + (amount > 1 ? 's' : '') + '!\n\nNow that is some hardcore crackling!\n⠀')
			...
			await client.pool.query('UPDATE user_data SET gold_coins = gold_coins + $1 WHERE Guild_ID = $3 AND Member_ID = ANY($2);', [amount, ids, guildId]);
			...
			const embed = new MessageEmbed()
				...
				// eslint-disable-next-line spaced-comment
				//.setThumbnail(user.defaultAvatarURL)
				.setDescription('⠀\nEveryone who has the ' + role.name + ' role got ' + amount + ' coin' + (amount > 1 ? 's' : '') + '!' + '\n\nNow that is some hardcore crackling!\n⠀')
				.setFooter('Check them out with /inventory!');

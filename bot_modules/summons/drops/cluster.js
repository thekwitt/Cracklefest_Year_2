...
module.exports = {
	async execute(interaction, client, channel_id, messageSpawn) {
		...
		try{ interactionMessage = await channel.send({ embeds: [embed] }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'First Send'));}
		catch (err) {
		...
		if(setting.delete_ot) {
			...
			catch (error) { client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Delete Denied'); }
		}
		...
		try{await interactionMessage.edit({ embeds: [embed], components: row }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'Expired Edit'));}
		catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Edit Denied - ' + String(err));}
		...
		if(filter == undefined) {
			messageSpawn.set('messageCount', setting.drop_message_count).set('timestamp', Math.floor(Date.now() / 1000) + setting.drop_time_count).set('activeMessage', false);
			return client.messages.set(interaction.guildId, messageSpawn);
		}
		...
		collector.on('collect', async i => {
			...
			if (ids.includes(i.user.id)) {
				try{await i.reply({ content: 'You already tried to get an egg cluster!', ephemeral: true }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'Already egg cluster Reply'));}
				catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Reply Denied - ' + String(err));}
			}
			else if (collector.users.size > setting.drop_obtain_count) {
				try{await i.reply({ content: 'Oh no! You were too late! Someone grabbed the last egg cluster.', ephemeral: true }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'Too Late egg cluster Reply'));}
				catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Reply Denied - ' + String(err));}
			}
			else if (i.customId === 'win') {
				ids.push(i.user.id);
				wins.push(i.user.username);
				await client.pool.query('UPDATE user_data SET Clusters = Clusters + 1 WHERE Guild_ID = $2 AND Member_ID = $1;', [i.user.id, interaction.guildId]);
				await client.pool.query('UPDATE user_stats SET Clusters_Collected = Clusters_Collected + 1 WHERE Guild_ID = $2 AND Member_ID = $1;', [i.user.id, interaction.guildId]);
				...
				try{await i.reply({ embeds: [embed], ephemeral: true }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', i.user.username + ' - egg cluster Reply'));}
				catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Got Reply Denied - ' + String(err));}

			} else {
				ids.push(i.user.id);
				loses.push(i.user.username);

				const data_u = await client.pool.query('SELECT * FROM user_data WHERE member_id = $1 AND Guild_ID = $2;', [i.user.id, interaction.guildId]);
				...
				await client.pool.query('UPDATE user_data SET basket_eggs = $1 WHERE Guild_ID = $3 AND Member_ID = $2;', [user_bag.basket_eggs, i.user.id, interaction.guildId]);
				...
				try{await i.reply({ embeds: [embed], ephemeral: true }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'egg cluster Failed Reply'));}
				catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Miss Reply Denied - ' + String(err));}
			}
		...
		try {
			...
			collector.on('end', async i => {
				...
				if(collector.users.size < setting.drop_obtain_count) {
					...
					try{await interactionMessage.edit({ embeds: [embed], components: [] }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'Expired Edit'));}
					catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Edit Denied - ' + String(err));}
				}
				else {
					embed = new MessageEmbed()
					...
					//.setThumbnail(user.defaultAvatarURL)
						.setDescription('⠀\n' + print_users + '**Keep on talking for another one to appear!**\n⠀')
						.setFooter('Each drop only lasts ' + setting.drop_duration + ' seconds!');
					try{await interactionMessage.edit({ embeds: [embed], components: [] }).then(client.extra.log_g(client.logger, interaction.guild, 'Message Create Event', 'Empty Edit'));}
					catch (err) {client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Edit Denied - ' + String(err));}
				}
			...
			client.extra.reloadMessageDrop(interaction.guild, client);
			client.extra.log_error_g(client.logger, interaction.guild, 'Message Create Event', 'Edit Failed From No Cache');
		}
	},
};
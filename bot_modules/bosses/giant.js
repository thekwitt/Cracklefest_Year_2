...
module.exports = {
	...
	async execute(message, client, channel_id, messageSpawn) {
		...
		embed = new MessageEmbed()
			...
			.setDescription('*It lowers it\'s head to you. You see the angry expression from the it\'s face.*')
				...
				if(damage < health && collector.ended == false) {
					if(second >= sec_limit && cast < 1) {
						...
						for(const user of tracked) {
							if(cast_yes.includes(user) == false) {
								try{ cast_name.push(message.guild.members.cache.find(user).username); } catch{;}
